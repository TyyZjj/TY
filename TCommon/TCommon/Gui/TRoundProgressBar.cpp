/*
 * TRoundProgressBar - a circular progress bar Qt widget.
 *
...
#include "TRoundProgressBar.h"

#include <QtGui/QPainter>
#include <QPainterPath>


TRoundProgressBar::TRoundProgressBar(QWidget *parent) :
    QWidget(parent),
    m_min(0), m_max(100),
    m_value(25),
...
void TRoundProgressBar::setRange(double min, double max)
{
    m_min = min;
    m_max = max;
...
void TRoundProgressBar::setMinimum(double min)
{
    setRange(min, m_max);
}

void TRoundProgressBar::setMaximum(double max)
{
    setRange(m_min, max);
}

void TRoundProgressBar::setValue(double val)
{
    if (m_value != val)
    {
        if (val < m_min)
...
void TRoundProgressBar::setValue(int val)
{
    setValue(double(val));
}

void TRoundProgressBar::setNullPosition(double position)
{
...
void TRoundProgressBar::setBarStyle(TRoundProgressBar::BarStyle style)
{
    if (style != m_barStyle)
    {
        m_barStyle = style;
...
void TRoundProgressBar::setOutlinePenWidth(double penWidth)
{
...
void TRoundProgressBar::setDataPenWidth(double penWidth)
{
...
void TRoundProgressBar::setDataColors(const QGradientStops &stopPoints)
{
...
void TRoundProgressBar::setFormat(const QString &format)
{
    if (format != m_format)
    {
        m_format = format;
...
void TRoundProgressBar::resetFormat()
{
...
void TRoundProgressBar::setDecimals(int count)
{
...
void TRoundProgressBar::paintEvent(QPaintEvent* /*event*/)
{
    ...
    painter.fillRect(QRectF(0, 0, width(), height()), palette().background());
    painter.drawImage((width() - outerRadius) / 2, (height() - outerRadius) / 2, buffer);
}

void TRoundProgressBar::drawBackground(QPainter &p, const QRectF &baseRect)
{
    p.fillRect(baseRect, palette().background());
}

void TRoundProgressBar::drawBase(QPainter &p, const QRectF &baseRect)
{
    switch (m_barStyle)
    {
    case StyleDonut:
...
void TRoundProgressBar::drawValue(QPainter &p, const QRectF &baseRect, double value, double arcLength)
{
    // nothing to draw
    if (value == m_min)
        return;
...
void TRoundProgressBar::calculateInnerRect(const QRectF &/*baseRect*/, double outerRadius, QRectF &innerRect, double &innerRadius)
{
    // for Line style
    if (m_barStyle == StyleLine)
    {
...
void TRoundProgressBar::drawInnerBackground(QPainter &p, const QRectF &innerRect)
{
...
void TRoundProgressBar::drawText(QPainter &p, const QRectF &innerRect, double innerRadius, double value)
{
    if (m_format.isEmpty())
        return;
...
QString TRoundProgressBar::valueToText(double value) const
{
...
void TRoundProgressBar::valueFormatChanged()
{
    m_updateFlags = 0;
...
void TRoundProgressBar::rebuildDataBrushIfNeeded()
{
    if (m_rebuildBrush)
    {
        m_rebuildBrush = false;

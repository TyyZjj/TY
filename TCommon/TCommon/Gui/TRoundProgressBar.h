...
#ifndef TROUNDPROGRESSBAR_H
#define TROUNDPROGRESSBAR_H

#include <QWidget>

/**
 * @brief The TRoundProgressBar class represents a circular progress bar and maintains its API
 * similar to the *QProgressBar*.
 *
 * ### Styles
 * TRoundProgressBar currently supports Donut, Pie and Line styles. See setBarStyle() for more details.
 *
 * ### Colors
 * Generally TRoundProgressBar uses its palette and font attributes to define how it will look.
 *
 ...
 *
 * ### Value text
 * Value text is generally drawn inside the TRoundProgressBar using its `font()` and \a QPalette::Text role from its `palette()`.
 *
 ...
 */
class TRoundProgressBar : public QWidget
{
    Q_OBJECT
public:    
    explicit TRoundProgressBar(QWidget *parent = 0);

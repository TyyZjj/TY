...
* 1.线程所在处理器
* 2.获取当前系统中所有的进程Id
* 3.是否是32位进程运行于64位操作系统中
* 4.程序开机自启
* 5.是否在整个系统运行的内存中是唯一的
* 
* 进程线程模块信息
* 1.根据进程名称查找进程id
* 2.根据进程Id查找其进程名(.exe的名称)
* 3.根据进程名称查找其内部线程id
* 4.获取进程下所有的模块名
* 5.获取进程下所有的模块信息
* 6.根据进程ID和窗口标题查找窗口句柄
...
* 2.获取CPU利用率(百分比)
* 3.获取进程的CPU利用率(百分比)
* 4.设置进程(线程)使用最多使用的核心数, 指定进程线程使用哪个/哪些CPU
* 
* IO相关
* 
* 内存相关
...
#pragma region 进程线程模块信息
	...
	/**
	 * @brief 根据进程ID和窗口标题查找窗口句柄
	 *
	 * 该函数通过枚举所有顶层窗口，查找属于指定进程且标题匹配的窗口。
	 * 如果szTitle为空字符串，则返回该进程的第一个窗口句柄。
	 *
	 * @param dwProcessId 目标进程ID
	 * @param szTitle 目标窗口标题（ANSI字符串），空字符串表示匹配任意标题
	 * @param lstTitle [out] 返回该进程所有枚举到的窗口标题列表
	 * @return HWND 找到的窗口句柄，未找到返回NULL
	 */

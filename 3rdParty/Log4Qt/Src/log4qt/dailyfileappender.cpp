...
QDateTime DefaultDateRetriever::currentDate() const
{
        ...
        const QString &originalFilename)
{
    ...
    for (const auto &fileName : logFileNames)
    {
        ...
        if (match.hasMatch())
        {
            const auto creationDate(QDateTime::fromString(match.captured(1), datePattern));
...
void DailyFileAppender::append(const LoggingEvent &event)
{
    ...
    if (currentDate.toString("yyyyMMddhh") != mLastDate.toString("yyyyMMddhh"))
    {
        rollOver();

	...
	: QDialog(parent)
{
	...
	if (type == nLoginUiType::nLoginUiTechnicianInput)
	{
		...
		this->setWindowTitle(QApplication::translate("LoginManager", "Technician login", nullptr)/*QString::fromUtf8("技术员登录")*/);
	...
	else if (type == nLoginUiType::nLoginUiEngineerInput)
	{
		...
		this->setWindowTitle(QApplication::translate("LoginManager", "Engineer login", nullptr)/*QString::fromUtf8("工程师登录")*/);
	...
	else if (type == nLoginUiType::nLoginUiAdminInput)
	{
		...
		this->setWindowTitle(QApplication::translate("LoginManager", "Admin login", nullptr)/*QString::fromUtf8("管理员登录")*/);
	...
	else if (type == nLoginUiType::nLoginUiNewUserInput)
	{
		...
		this->setWindowTitle(QApplication::translate("LoginManager", "Add new user", nullptr)/*QString::fromUtf8("添加用户")*/);
		
		showShowLayout(false);
		showOperateLayout(false);
		
		ui->authorityBox->addItems(QStringList() 
			<< getDefaultUserGroupName(DUGL::nUserLevel_Operator)
			<< getDefaultUserGroupName(DUGL::nUserLevel_Technician)
			<< getDefaultUserGroupName(DUGL::nUserLevel_Engineer)
			<< getDefaultUserGroupName(DUGL::nUserLevel_Supervisor)
			<< getDefaultUserGroupName(DUGL::nUserLevel_Admin));
	...
	else if (type == nLoginUiType::nLoginUiEditUserInput)
	{
		...
		this->setWindowTitle(QApplication::translate("LoginManager", "Edit user", nullptr)/*QString::fromUtf8("编辑用户")*/);
	...
	else if (type == nLoginUiType::nLoginUiInputUserInput)
	{
		...
		this->setWindowTitle(QApplication::translate("LoginManager", "Input user id", nullptr)/*QString::fromUtf8("输入用户名")*/);
	...
	else if (type == nLoginUiType::nLoginUiDelUserInput)
	{
		...
		this->setWindowTitle(QApplication::translate("LoginManager", "Delete user", nullptr)/*QString::fromUtf8("删除用户")*/);
	...
	else
	{
		...
		this->setWindowTitle(QApplication::translate("LoginManager", "Login", nullptr)/*QString::fromUtf8("登录")*/);
...
{
	if (m_iType == nLoginUiType::nLoginUiEditUserInput)//编辑用户时, 用户的主用户名称不能被编辑
	...
	ui->authorityBox->setCurrentText(loginUser.getUserGroup());
...
void CLoginUI::on_okBtn_clicked()
{
		...
		m_iType != nLoginUiType::nLoginUiInputUserInput)
	{
		...
		{
			showShowLayout(true);
			ui->showLbl->setText(QApplication::translate("LoginManager", "User name/Password is null.", nullptr)/*QString::fromUtf8("用户名/密码不可为空!")*/);
			return;
		}
	}
	...
	else if (m_iType == nLoginUiType::nLoginUiTechnicianInput)
	{
		...
		ret = isDefaultUserGroup(strUserGroup);
		int iLevel = getDefaultUserGroupLevel(strUserGroup);
		if (!ret)
		{
			ui->showLbl->setText(QApplication::translate("LoginManager", "Not support add new user group.", nullptr)/*QString::fromUtf8("暂时未实现添加用户组!")*/);
			return;
		}
		...
		else if (iLevel > DUGL::nUserLevel_Invalid)
		{
			ui->showLbl->setText(QApplication::translate("LoginManager", "%1 no permission.", nullptr)/*QString::fromUtf8("%1权限不足!")*/.arg(strUser));
			showShowLayout(true);
			return;
		}
		else
		{
			ui->showLbl->setText(QApplication::translate("LoginManager", "User not exist.", nullptr)/*QString::fromUtf8("不存在当前用户!")*/);
			showShowLayout(true);
			return;
		}
	}
	else if (m_iType == nLoginUiType::nLoginUiEngineerInput)
	{
		...
		if (iLevel >= DUGL::nUserLevel_Engineer)
		{
			QString strResult;
			bool ret = CLoginManager::GetKernel()->Check(strUser, strPassword);
			if (ret)
	...
	else if (m_iType == nLoginUiType::nLoginUiAdminInput)
	{
		...
		if (iLevel >= DUGL::nUserLevel_Admin)
		{
			bool ret = CLoginManager::GetKernel()->Check(strUser, strPassword);
			if (ret)
			{
				ui->showLbl->clear();
				qDebug() << QApplication::translate("LoginManager", "User %1 check success.", nullptr)/*QString::fromUtf8("用户 %1 验证成功.")*/.arg(strUser);
				accept();
			}
			else
			{
	...
	else if (m_iType == nLoginUiType::nLoginUiNewUserInput)
	{
		...
		{
			ui->showLbl->setText(QApplication::translate("LoginManager", "Two password inputs are inconsistent.", nullptr)/*QString::fromUtf8("两次密码输入不一致.")*/);
			showShowLayout(true);
			return;
		}
		...
		{
			ui->showLbl->setText(QApplication::translate("LoginManager", "Password tips can not contain password.", nullptr)/*QString::fromUtf8("密码提示不能包含密码信息.")*/);
			showShowLayout(true);
			return;
		}
		...
		loginUser.setUserGroup(strAuthority);
		...
		if (ret)
		{
			QString strInfo = QApplication::translate("LoginManager", "User %1 add success.", nullptr)/*QString::fromUtf8("用户 %1 添加成功.")*/.arg(strUser);
			qDebug() << strInfo;
	...
	else if (m_iType == nLoginUiType::nLoginUiInputUserInput)
	{
		...
		{
			ui->showLbl->setText(QApplication::translate("LoginManager", "No permission.", nullptr)/*QString::fromUtf8("权限不足!")*/);
			showShowLayout(true);
			return;
		}
		if (iLevel <= DUGL::nUserLevel_Invalid)
		{
			ui->showLbl->setText(CLoginManager::GetKernel()->getLastError());
			showShowLayout(true);
			return;
	...
	{
		ui->showLbl->setText(QApplication::translate("LoginManager", "Invalid UI Type.", nullptr)/*QString::fromUtf8("无效的类型.")*/);
		showShowLayout(true);
		return;
	}
}
...
void CLoginUI::on_addUserIdBtn_clicked()
{
	...
	userIdTypeBox->addItems(QStringList() << UserIdType_Card//QString::fromUtf8("卡号")
		<< UserIdType_WorkId//QString::fromUtf8("工号")
		<< UserIdType_PhoneNumber//QString::fromUtf8("手机")
		<< UserIdType_Email//QString::fromUtf8("邮箱")
		<< UserIdType_QQ//QString::fromUtf8("QQ")
		<< UserIdType_WeChat/*QString::fromUtf8("微信")*/);
	userIdTypeBox->setObjectName(QString::fromUtf8("newUserIdTypeBox%1").arg(iCount));
	userIdTypeBox->setMinimumSize(QSize(90, 30));
	userIdTypeBox->setMaximumSize(QSize(90, 30));
...
void CLoginUI::on_editDBBtn_clicked()
{
	CLoginUI loginUi(nLoginUiType::nLoginUiAdminInput, this);
	if (loginUi.exec() == QDialog::DialogCode::Accepted)
	{
		CMySqlDBParaUI mysqlDBParaUI(this);
		mysqlDBParaUI.exec();
	}
}
...
bool CLoginUI::getLoginUser(sLoginUser &loginUser)
{
	...
	QString strPassword = ui->passwordEdit->text();
	QString strUserGroup = ui->authorityBox->currentText();
	QString strPasswordTips = ui->passwordTipsEdit->toPlainText();
	...
	loginUser.setPassword(strPassword, strPasswordTips);
	loginUser.setUserGroup(strUserGroup);
	QList<sUserId> lstUserId = loginUser.getUserIdList();
	for (auto userId : lstUserId)
	{
		if (userId.iUserIdType != sUserId::nUserIdType::nUserIdType_UserMainName &&
